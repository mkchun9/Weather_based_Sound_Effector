...
class WeatherSonificationApp(wx.Frame):
    ...
    def init_effects_chain(self):
        ...
        rev_size = 0.5 + (hour_data["humidity"] / 100) * 0.5  # 0.5 ~ 1.0
        
        # 저역 필터 컷오프 (온도)
        cutoff_freq = 300 + (hour_data["temp"] - 10) * 40  # 대략 300 ~ 1100Hz
        
        self.filter_lp = ButLP(self.music_player, freq=cutoff_freq)
        
        self.rev = Freeverb(self.filter_lp, size=rev_size, bal=rev_bal)
        
        # tremolo 속도 풍속에 비례, 진폭 고정
        trem_rate = 0.1 + (hour_data["wind_speed"] / 10) * 5  # 0.1 ~ 5Hz
        self.lfo = Sine(freq=0.1, mul=0.5, add=0.5)
        self.tremolo = self.rev * self.lfo
        
        # HRTF 위치: 풍향에 따른 위치 맵핑 (0~360도 -> 3D 방향)
        azimuth = hour_data["wind_deg"] % 360
        elevation = 0  # 평면상 (필요시 변경 가능)
    ...
    def run_sonification_loop(self):
        ...
        while self.running:
            ...
            # 부드러운 파라미터 변화 (wx.CallAfter + SigTo로 처리할 수도 있음)
