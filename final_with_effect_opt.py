...
class WeatherSonificationApp(wx.Frame):
    def __init__(self):
        super().__init__(None, title="Weather Data Sonification(Sound Design&Programming/20251144_MInkyoungChun)", size=(700, 900))
        ...
        self.region_list = wx.ListBox(panel, choices=["Seoul", "Gwangju", "LosAngeles", "Vancouver","Dynamic","Winter_Sample"])
        ...
        desc_sizer.Add(wx.StaticText(panel, label="• 기온 (Temperature): 음 높이 및 재생 속도 조절"), flag=wx.LEFT, border=10)
        desc_sizer.Add(wx.StaticText(panel, label="• 풍속 (Wind Speed): 팬 이동 속도 변화"), flag=wx.LEFT, border=10)
        ...
        desc_sizer.Add(wx.StaticText(panel, label="• 습도 (Humidity): 리버브 양 (울림 정도)"), flag=wx.LEFT, border=10)
        desc_sizer.Add(wx.StaticText(panel, label="• 자외선 지수 (UV Index): 하이 쉘프 EQ 볼륨"), flag=wx.LEFT, border=10)
        ...
        self.effect_status_text = wx.StaticText(panel, label="Effect Auto-Update: ON")
        ...
        # --- 버튼들 ---
        self.optimize_btn = wx.Button(panel, label="Find your optimized effect")
        self.fix_effect_btn = wx.Button(panel, label="Fix Effect Values")
        self.toggle_effect_btn = wx.Button(panel, label="Toggle Effect Auto-Update")
    ...
    def init_effects_chain(self):
        ...
        hour_data = self.weather_data["hourly"][0] if self.weather_data else {
            "temp": 25, "humidity": 50, "rain": 0, "snow": 0, "wind_speed": 1, "wind_deg": 0, "uvi": 0
        }
        rev_bal = min(hour_data["humidity"] / 100, 1) * 0.8
        rev_size = 0.5 + (hour_data["humidity"] / 100) * 0.5
        ...
        trem_rate = 0.1 + (hour_data["wind_speed"] / 10) * 5
        self.lfo = Sine(freq=0.1, mul=0.5, add=0.5)
        self.tremolo = self.rev * self.lfo
        azimuth = hour_data["wind_deg"] % 360
        elevation = 0
        self.hrtf = HRTF(self.tremolo, azimuth=azimuth, elevation=elevation)
        self.hrtf.out()
    ...
    def run_sonification_loop(self):
        ...
        while self.running:
            ...
            if self.auto_update_effects and self.fixed_hour is None:
                rev_bal = min(max(humidity / 150, 0.2), 0.7)
                rev_size = min(max(humidity / 120, 0.4), 0.9)
                ...
                rev_bal = self.optimized_effects.get("rev_bal", 0.5)
                rev_size = self.optimized_effects.get("rev_size", 0.7)
                cutoff_freq = self.optimized_effects.get("cutoff_freq", 600)
                trem_rate = self.optimized_effects.get("trem_rate", 1.0)
                eq_gain = self.optimized_effects.get("eq_gain", 0.5)
            ...
            self.update_graph(hour_to_use)
            self.update_weather_summary(hour_to_use)
    ...
    def on_open_optimizer(self, event):
        import effect_optimizer  # effect_optimizer.py 필요
    ...
    def on_fix_effects(self, event):
        ...
        self.fixed_hour = self.current_hour  # 이게 중요!
