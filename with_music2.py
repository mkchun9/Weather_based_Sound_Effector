...
class WeatherSonificationApp(wx.Frame):
    def __init__(self):
        super().__init__(None, title="Weather Sonification with Weather Summary", size=(700, 650))
        ...
        self.region_list = wx.ListBox(panel, choices=["Seoul", "Gwangju", "LosAngeles", "Vancouver"])
        ...
        self.weather_summary = wx.TextCtrl(panel, style=wx.TE_MULTILINE|wx.TE_READONLY, size=(-1,80))
    ...
    def init_effects_chain(self):
        ...
        # 초기 날씨 데이터가 없으면 기본값 사용
        hour_data = self.weather_data["hourly"][0] if self.weather_data else {
            "temp":20, "humidity":50, "wind_speed":2, "wind_deg":180, "rain":0, "snow":0, "uvi":0
        }
    ...
    def run_sonification_loop(self):
        while self.running:
            ...
            # 기온 → pitch 및 속도 변환 (speed)
            temp = hour_data["temp"]
            pitch_ratio = 0.5 + (temp - 10) / 40  # 0.5 ~ 1.5 범위
            
            # 풍속 → 팬 이동 속도 조절
            wind_speed = hour_data["wind_speed"]
            wind_deg = hour_data["wind_deg"]
            target_pan = (wind_deg % 360) / 360
            
            diff = target_pan - self.pan.pan
            self.pan.pan += diff * min(wind_speed / 10, 1) * 0.1
            self.pan.pan = max(0, min(1, self.pan.pan))
            
            # 강수량 + 적설량 → 볼륨
            rain = hour_data.get("rain", 0)
            snow = hour_data.get("snow", 0)
            vol = max((rain + snow) / 10, 1)
            
            # 습도 → 리버브
            humidity = hour_data["humidity"]
            rev_bal = min(humidity / 100, 1) * 0.8
            ...
            # 효과 적용
            if self.music_player:
                self.music_player.speed = pitch_ratio  # pitch & speed 같이 조절됨
                self.music_player.mul = vol * 0.8      # 볼륨 (강수+적설량 기반)
            if self.rev:
    ...
    def update_weather_summary(self, current_hour=0):
        ...
        summary = f"Hour {current_hour}: {desc.capitalize()}\n" \
